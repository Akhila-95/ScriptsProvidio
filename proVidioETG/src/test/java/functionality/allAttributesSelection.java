...
public class allAttributesSelection extends baseClass{
	...
	public static void selectTheAttributesInPdp() throws Exception {
		...
		allAttributes();
	    Thread.sleep(2000);	  	      
	     if(Actionsss.enabledElement(pdpPage.getSelectAddtoCartBtn())&& (Actionsss.elementSize(pdpPage.getInStockList()) 
	    		 ||Actionsss.elementSize(pdpPage.getPreOrderList()) || Actionsss.elementSize(pdpPage.getBackOrderList()))) {
	    	 ProductDetailPageValidation.verifyingEnablingOfDeliveryToAddress();
	    	 Actionsss.CombinedClick(pdpPage.getSelectAddtoCartBtn());	    	
	...
	public static void selectTheAttributesInPdpForProductSet() throws Exception {
	...
	public static void pickInStoreInPdp() throws Exception {
	    ...
	    Thread.sleep(2000);
	     ...
	     if(Actionsss.enabledElement(pdpPage.getSelectAddtoCartBtn())&& (Actionsss.elementSize(pdpPage.getInStockList()) 
	    	 ...
	    	 ProductDetailPageValidation.verifyingEnablingOfPickUpStore();
	    	 Actionsss.CombinedClick(pdpPage.getSelectAddtoCartBtn());	  
	 		 ProductListingPageValidations.validatingProductisAddtoCart();   
	     }else {
	    	 reSelectTheProduct();
	...
	public static void reSelectTheProduct() throws Exception {
		 logger.info("Product is out of stock so searching for new product");
    	 Actionsss.CombinedClick(pdpPage.getLastBreadcrumb());
	...
	public static void allAttributes() throws Exception {
		...
		List<WebElement> colorElement = driver.findElements(By.xpath("//div[contains(@class,'product-detail set-item')]//select[contains(@class,'select-color-swatch')]"));
	...
	public static void allAttributesForProductSet() throws Exception {
	    ...
	    //another way of attributes 
	     ...
	     for(int i=1; i<=mainDiv.size();i++) {
				if(colorElement.size()>0|| colorBox.size()>0) {
					...
					test.pass("Successfully color attribute is seleccted from pdp page");
					Thread.sleep(1000);
				}else if(sizeElement.size()>0|| sizeBox.size()>0 ) {
					test.info("Verify whether the size attribute is able to select from PDP page.");
					attributesSelection.sizeSelction();
					logger.info("selected size");
					test.pass("Successfully size attribute is seleccted from pdp page");
					Thread.sleep(1000);					
				}else  if(memoryElement.size()>0||  memoryBox.size()>0) {
					test.info("Verify whether the memory attribute is able to select from PDP page.");
					attributesSelection.memorySelection();
					logger.info("selected memory size");
					test.pass("Successfully memory attribute is seleccted from pdp page");
					Thread.sleep(1000);
				} else if(widthElements.size()>0 || widthBox.size()>0) {	
					test.info("Verify whether the width attribute is able to select from PDP page.");
					attributesSelection.widthSelection();
					logger.info("selected width");
					test.pass("Successfully width attribute is seleccted from pdp page");
					Thread.sleep(1000);
				}else if(showName.size()>0 || showNameBox.size()>0) {
					test.info("Verify whether the show name attribute is able to select from PDP page.");
					Thread.sleep(1000);
					attributesSelection.selectShowName();
					logger.info("Selected show name ");
	...
	public static void selectTheAttributesInQuickShop_POP_UP() throws Exception {
		//select the filters
		quickshopallAttributes();
		Thread.sleep(3000);
		List<WebElement> InStock = driver.findElements(By.xpath("//div[text()='In Stock']"));
		if(InStock.size()>0) {
			...
			selectTheAttributesInQuickShop_POP_UP();
		}
	}
	...
	public static void quickshopallAttributes() throws Exception {
		...
		List<WebElement> memoryElement = driver.findElements(By.xpath("//select[contains(@class,'select-memorySize')]"));
	...
	public static void selectTheAttributesInPdpForBuyNow() throws Exception {
		...
		//selecting attributes
	     ...
	     if(cartEnabled.isEnabled()&& inStock.size()>0) {
	    	Actionsss.CombinedClick(pdpPage.getBuyNowButton());
	     }else {
